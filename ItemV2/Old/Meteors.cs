    ...
    public class Meteors : HurtworldPlugin
    {
        ...
        public class MeteorInstance
        {
            ...
            public MeteorInstance(float heatTimeDuration, bool spawnLoot, bool destroyOnFall, FallSiteInfo? fallSite)
            {
                ...
                //this.gameObject.GetComponent<MeteorEvent>().MapMarker.d
            ...
            public IEnumerator UpdateMeteor(float interval)
            {
                try
                {
                    ...
                    while (true)
                    {
                        ...
                        if (currentSquareDistance <= 20 * 20)
                            break;
                    ...
                    Meteors.Instance.Server.Broadcast(string.Format(Meteors.Instance.lang.GetMessage(msg_meteor_hot, Meteors.Instance), this.heatDuration.ToString()));
                    ...
                    Meteors.Instance.Server.Broadcast(string.Format(Meteors.Instance.lang.GetMessage(msg_meteor_cold, Meteors.Instance), this.heatDuration));
            ...
            public void SpawnLoot()
            {
                ...
                foreach (MeteorLootItem mli in Meteors.Instance.lootItemsDB.lootItems)
                {
                    if (UnityEngine.Random.Range(0.0f, 1.0f) >= mli.chance * globalRate)
                        continue;
                    ItemGeneratorAsset generator = Singleton<GlobalItemManager>.Instance.GetGenerators()[mli.itemID];
                    ItemObject item = Singleton<GlobalItemManager>.Instance.CreateItem(generator, UnityEngine.Random.Range(mli.minStackSize, mli.maxStackSize));
                    GlobalItemManager.SpawnWorldItemNice(item,
            ...
            public void Destroy(bool removeFromList)
            {
                ...
                if (this.gameObject != null)
                {
                    ...
                    HNetworkView nwv = HNetworkExtensions.HNetworkView(this.gameObject);
            ...
            {
                GameObject explosionGO = Singleton<HNetworkManager>.Instance.NetInstantiate(Prefabs.GetKey("explosionserver"), pos, Quaternion.identity, GameManager.GetSceneTime());
        ...
        public class MeteorsFallSitesDB
        {
            ...
            {
                this.Add("-2068.78 182.875 1208.731;800;в СНЕЖНОМ БИОМЕ");
                this.Add("-2674.589 283.6281 -813.9224;800;в ЛЕСУ");
                this.Add("-645.3057 241.7302 -2214.017;700;в ПУСТЫНЕ");
                this.Add("1233.461 299.4619 -1884.715;1000;в КРАСНОЙ ПУСТЫНЕ");
                this.Add("-2678.557 269.2448 -2528.645;50;на АРКУ");
                this.Add("-915.2287 233.812 -2438.202;150;около КОРАБЛЯ");
                this.Add("1475.228 204.7502 -1899.655;200;около ПЕРЕКРЕСТКА");
                this.Add("1124.911 194.4053 -3352.688;200;около АЭРОДРОМА");
                this.Add("-1269.364 178.0141 1185.167;200;около ДЫРЫ");
                this.Add("-2603.134 178.2825 1609.538;200;около ТРАНЗИТА");
                this.Add("-2378.846 200.2309 -927.0706;150;около КРЕПОСТИ");
                this.Add("-3319.789 202.7251 -1482.855;200;в ДОЛИНЕ");
            }
        }
        ...
        public ConfigurationField_Int cfg_MeteorsHeatDurationThrow;
        ...
        private void Init()
        {
            ...
            this.cfg_MeteorsHeatDurationThrow = new ConfigurationField_Int(this.Config, "MeteorsHeatDurationThrow", 1);
        ...
        public void ScheduleMeteor()
        {
            ...
            Server.Broadcast(string.Format(lang.GetMessage(msg_meteor_earlyfallwarning, this), futureFallSite.Name, earlyAnnouncementTime));
        ...
        public void ThrowMeteorAtSite(FallSiteInfo fallSite)
        {
            ...
            Server.Broadcast(string.Format(lang.GetMessage(msg_meteor_located_fallwarning, this), fallSite.Name));
        ...
        {
            Server.Broadcast(string.Format(lang.GetMessage(msg_throwmeteor_fall_warning, this), targetPlayer.Identity.Name));
            float speed = 100f;
            ...
            MeteorInstance TnewMeteor = new MeteorInstance(this.cfg_MeteorsHeatDurationThrow.value, true, false, null);
            TnewMeteor.Fire(starting, final, speed);
        }
        ...
        public PlayerSession FindPlayer(string name)
        {
            ...
            foreach (KeyValuePair<uLink.NetworkPlayer, PlayerSession> entry in sessions)
                if (entry.Value != null)
                    if (entry.Value.Identity.Name.ToLower().Contains(name.ToLower()))
                        return entry.Value;
            return session;
        }
        ...
        private object OnServerCommand(string arg)
        {
            ...
            if ((args[0].ToLower() == "throwmeteor"))
            {
                ...
                {
                    var nik = pair.Value;
                    if (Player == Convert.ToString(getSession(nik.Identity.Name).SteamId))
                        Player = nik.Identity.Name;
                }

                PlayerSession target = getSession(Player);
                if (target == null)
                {
        ...
        void cmdThrowMeteor(PlayerSession session, string command, string[] args)
        {
            ...
            this.ThrowMeteorAtPlayer(target);
        }

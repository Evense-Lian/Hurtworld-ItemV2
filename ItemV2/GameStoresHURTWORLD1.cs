...
namespace Oxide.Plugins
{
    ...
    class GameStoresHURTWORLD1 : HurtworldPlugin
    {
                        ...
                        switch (Convert.ToInt32(Response["code"]))
                        {
                            case 100:
                                ...
                                foreach (object pair in data)
                                {
                                    ...
                                    {
                                        string command = iteminfo["command"].ToString().Replace('\n', '|').ToLower().Trim('\"').Replace("%steamid%", Player.SteamId.m_SteamID.ToString()).Replace("%username%", Player.Identity.Name);
                                        String[] CommandArray = command.Split('|');
                                        foreach (var substring in CommandArray)
                                        {
                                            if (substring.ToLower().Contains("additem"))
                                            {
                                                OnServerCommand(substring);
                                            }
                                            else
        ...
        void OnServerCommand(string cmd)
        {
            ...
            try
            {
                if (args[0].ToLower() == "additem")
                {
                    ...
                    Match match = pattern.Match(cmd);
                    var ItemMgr = Singleton<GlobalItemManager>.Instance;
                    var item = GetItem(match.Groups[0].Value, Convert.ToInt32(args[2]));
                    ItemMgr.GiveItem(Player.Find(args[1]).Player, item.Generator, Convert.ToInt32(args[2]));
                }
            }
            catch
            {

...
namespace Oxide.Plugins
{
    ...
    public class ExtTeleport : HurtworldPlugin
    {
        ...
        static public List<ulong> cancelTeleports = new List<ulong>();
        static public Dictionary<ulong, float> cancelHealth = new Dictionary<ulong, float>();
            ...
            Vector3 position, bool initial)
        {
            ...
            string status = _helpers.getStatus(s1, s2);
            ...
            {
                SendChatMessage(s1, GetMsg("Prefix"), reasonMessage.Replace("{reason}", "Отменен игроком " + status));
                if(teleportType == ETeleportType.Player)
                    SendChatMessage(s2, GetMsg("Prefix"), reasonMessage.Replace("{reason}", "Отменен игроком " + status));
                return false;
            }
        ...
        internal class Helpers
        {
            ...
            public string getStatus(PlayerSession s1,PlayerSession s2)
            {
                try
                {
                    if (cancelTeleports.Contains((ulong)s1.SteamId))
                    {
                        cancelTeleports.Remove((ulong)s1.SteamId);
                        return s1.Identity.Name;
                    }

                    if (cancelTeleports.Contains((ulong)s2.SteamId))
                    {
                        cancelTeleports.Remove((ulong)s2.SteamId);
                        return s2.Identity.Name;
                    }
                }
                catch
                {
        ...
        [ChatCommand("tpc")]
        private void CommandPlayerReset(PlayerSession session, string command, string[] args)
        {
            ...
            cancelTeleports.Add((ulong)session.SteamId);
        }
